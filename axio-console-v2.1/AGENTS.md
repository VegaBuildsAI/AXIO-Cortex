...
> **Codex context file.** Read this first before touching any file in this project.
...
open-source LLMs as the default backend, with the Anthropic Codex API as the premium cloud tier.
...
- `py axio.py --Codex <mode>` — force all prompts through Codex Sonnet for the session

---

## Four Operational Modes

| Mode    | Codex Model              | Local Model         | Purpose |
|---------|---------------------------|---------------------|---------|
| Chat    | Codex-haiku-4-5-20251001 | mistral:latest      | Fast general-purpose conversation with session memory |
| Code    | Codex-sonnet-4-6         | qwen3-coder:30b     | Autonomous coding agent with file tools + PowerShell |
| Cowork  | Codex-sonnet-4-6         | auto-routed         | File-aware workspace assistant with smart routing |
| RevRec  | Codex-sonnet-4-6         | qwen3:14b           | ASC 606 / IFRS 15 revenue recognition specialist |
...
```
axio-console-v2.1/
├── AGENTS.md                          ← THIS FILE (Codex reads on startup)
...
```bash
# Required
ANTHROPIC_API_KEY=sk-ant-api03-...       # Codex API key (also accepted as CLAUDE_API_KEY)

# Models (all have safe defaults)
CLAUDE_MODEL=Codex-sonnet-4-6           # Default for Code, Cowork, RevRec
CLAUDE_MODEL_CHAT=Codex-haiku-4-5-20251001  # Chat mode only (cheaper/faster)
...
| # | Issue | Fix |
|---|-------|-----|
| 1 | Codex model hardcoded to `Codex-opus-4-1-20250805` | Configurable via `CLAUDE_MODEL` in `.env` |
...
- **Per-mode Codex model selection**: `ClaudeClient.__init__` accepts optional `model: str`.
  Chat mode passes `CLAUDE_MODEL_CHAT` (Haiku). All other modes inherit `CLAUDE_MODEL` (Sonnet).
  Defined in `core/config.py`:
  ```python
  CLAUDE_MODEL      = os.getenv("CLAUDE_MODEL", "Codex-sonnet-4-6")
  CLAUDE_MODEL_CHAT = os.getenv("CLAUDE_MODEL_CHAT", "Codex-haiku-4-5-20251001")
  ```

- **`--Codex` flag**: Forces all prompts through Codex Sonnet for the entire session.
...
- [x] `modes/code.py` — Code mode agentic loop, file tools, PowerShell, Codex force flag
...
1. ~~**Wire `math_tool.py` into router**~~ — **DONE (May 9, 2026)**
   ...
   before any Ollama/Codex call. MATH-001 confirmed 100/100 in benchmark.
...
5. ~~**Update `.env.example`**~~ — **DONE (May 9, 2026)**
   Now references `Codex-sonnet-4-6` and `Codex-haiku-4-5-20251001` with full Ollama
...
4. **Hardcoded model names (FIXED)**: Previously hardcoded to `Codex-opus-4-1-20250805`
...
- **`FORCE_CLAUDE` env var**: If set to `"1"`, all modes bypass local routing and send
  every prompt to Codex. Set by `axio.py` when `--Codex` flag is passed.
